...
{
    internal class CServer : NetServer
    {
        ...
        public static void GotMessage(object peer)
        {
            ...
            switch (msg.MessageType)
            {
                case NetIncomingMessageType.Error:
                    ...
                    Forms.Room2_host.Log("[SERVER ERROR] " + (worked ? errorText : "(???)"));
                    break;
                ...
                case NetIncomingMessageType.StatusChanged:
                    ...
                    if (status == NetConnectionStatus.Disconnected)
                    {
                        ...
                        if (n != null)
                        {
                            Forms.Room2_host.Log("User '" + n + "' has disconnected from the server. Cya!");
                            server.SetName(msg.SenderConnection, null);
                            Forms.Room2_host.RemoveUser(n);
                    ...
                    if (status == NetConnectionStatus.Connected)
                    {
                        ...
                        outMsg.Write(true);
                        outMsg.Write(Forms.Room2_host.ActiveUsername);
                ...
                case NetIncomingMessageType.WarningMessage:
                    Forms.Room2_host.Log("[SERVER WARNING] " + msg.ReadString());
                    break;

                case NetIncomingMessageType.ConnectionApproval:
                    string name = msg.ReadString().Trim();
                    if (server.names.ContainsValue(name) || Forms.Room2_host.ActiveUsername == name)
                    {
                        msg.SenderConnection.Deny("Somebody else on the server already has your username.");
                        Forms.Room2_host.Log("Rejected connection from " + msg.SenderConnection + " because they have a duplicate username: " + name);
                    }
                    else
                    {
                        ...
                        Forms.Room2_host.Log("Approved connection from " + msg.SenderConnection.RemoteEndPoint + ", called '" + name + "', and sent the current names. (" + (server.names.Count + 1) + ")");
                    }
                    break;

                default:
                    Forms.Room2_host.Log("[SERVER UNHANDLED] Type: " + msg.MessageType);
                    break;
            }
        ...
        protected void SetName(NetConnection n, string name)
        {
            ...
            else
            {
                ...
                Forms.Room2_host.Log("User @ " + n.RemoteEndPoint + " has been assigned name '" + name + "'");
        ...
        {
            if (name == Forms.Room2_host.ActiveUsername)
            {
                return LocalMuted;
            }
